...
# Standardize residuals to identify true outliers
schools_pr$z_score <- as.numeric(scale(schools_pr$resid))

# Schools performing significantly better than expected (z > 2)
outliers_positive <- schools_pr %>%
  filter(z_score > 2) %>%
  select(School, County, poorpct, p_passed, pred, resid, z_score) %>%
  arrange(desc(z_score))

kable(head(outliers_positive, 10),
      digits = 2,
      caption = "Schools performing significantly above expectations (z-score > 2)")
```

```{r outliers-negative}
# Schools performing significantly below expected (z < -2)
outliers_negative <- schools_pr %>%
  filter(z_score < -2) %>%
  select(School, County, poorpct, p_passed, pred, resid, z_score) %>%
  arrange(z_score)

kable(head(outliers_negative, 10),
      digits = 2,
      caption = "Schools performing significantly below expectations (z-score < -2)")
```


## Key Takeaways

1. **GNI is a powerful predictor**: A single variable (GNI) explains 87% of variation in health expenditure across countries.
2. **Adding variables isn't always better**: Unemployment added no predictive value to the model.
3. **Poverty strongly predicts academic outcomes**: In California schools, poverty rate is significantly associated with lower pass rates.
4. **Outlier schools tell stories**: Residual analysis reveals schools that defy expectations — both positively and negatively — which can guide further reporting and investigation.
